...
export function errorHandler(error: any, request: FastifyRequest, reply: FastifyReply) {
	...
	switch (true) {
		...
		case error instanceof SyntaxError:
			request.log.fatal(error, error.message);
			problem = new Problem(400, ReasonPhrases.BAD_REQUEST.toUpperCase(), null, {
				detail: error.message,
			});
			break;
		...
		default:
			...
			problem = new Problem(500, ReasonPhrases.INTERNAL_SERVER_ERROR.toUpperCase(), null, {
	...
	return reply.code(problem.status).type("application/json").send(JSON.stringify(problem));
}
